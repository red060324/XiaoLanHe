...
- Status: `IMPLEMENTATION VERIFIED — LOCAL/REMOTE CI PASS; EXTERNAL GATES BLOCKED`
...
| T11 | PRE_MERGE | PARTIAL | Add pinned one-replica official LightRAG service to local/CI Compose with two Gunicorn workers, four-store config using Milvus for vector projections, private strong API key/network, route-tiered request limits, memory/volume limits and persistent `WORKING_DIR` | AC5, AC6, AC15 | current bootstrap/live/RBAC smoke passes in run `34274141157`; full restart/backup/restore lifecycle remains blocked |
...
| T18 | PRE_MERGE | BLOCKED | Complete docs/rollback and all focused/race/MySQL/frontend/architecture/container/full CI gates including LightRAG/Milvus restart/restore | AC14, AC15 | current local CI and GitHub run `34274141157` at `583f42a` pass; full official lifecycle/restore remains externally blocked |
