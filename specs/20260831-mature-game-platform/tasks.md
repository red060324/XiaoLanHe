...
| T39 | PRE_MERGE | IN_PROGRESS | Publish the latest audited revision and verify clean-checkout GitHub Actions | AC10 | clean local clone of `75c8a03` passed full `make ci` against fresh PostgreSQL 17 + pgvector after lockfile install; exact remote push authorization and latest GitHub Actions run pending |
