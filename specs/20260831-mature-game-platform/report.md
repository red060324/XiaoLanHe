...
| Latest clean-clone command | committed `75c8a03`; local no-hardlink clone, lockfile `npm ci`, fresh PostgreSQL 17 + pgvector, then the same full `make ci BASE_REF=origin/master` | PASS |
...
| Final documentation and latest code-bearing clean-checkout CI | VERIFYING | T39; committed `75c8a03` passes a clean local clone, latest GitHub Actions still requires the authorized push |
