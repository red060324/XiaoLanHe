...
|---|---|---|
| Latest local full command | T60 `XLH_TEST_DATABASE_URL=<isolated PostgreSQL 17 database URL> make ci BASE_REF=origin/master`: Go vet/tests/race against PostgreSQL 17 + pgvector, 50 frontend tests/build, hooks, architecture and spec drift | PASS |
...
| PostgreSQL | Isolated local PostgreSQL 17 + pgvector 0.8.6 and GitHub Actions `33377050477` passed migrations plus identity, literal Catalog search, forum, hidden-post final comment/reaction rejection, promotion, available-coupon reservation, final campaign/price order revalidation, and entitlement integration | PASS |
...
| Search input boundary | query trim/blank/100-rune checks, literal Catalog `%`/`_` semantics, and knowledge limit 1-10; invalid requests prove zero downstream calls | PASS |
...
| All product PRE_MERGE behavior | PASS | T1-T15 and T20-T60 pass |
