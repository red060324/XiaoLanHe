...
|---|---|---|
| Latest local full command | `make ci BASE_REF=origin/master` on the T34 code-bearing working tree: Go tests, full race, 29 frontend tests/build, hooks, architecture and spec drift | PASS |
...
| Frontend | 4 files, 29 Vitest tests and Vite production build; latest-request catalog/community state, coupon-claim restoration/reservation, account-isolated chat history, failed-stream and mixed-edition ownership regressions | PASS |
...
| All product PRE_MERGE behavior | PASS | T1-T15 and T20-T34 |
