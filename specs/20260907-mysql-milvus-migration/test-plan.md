...
| V17 | PRE_MERGE | release workers | generated claimable time axis, bounded multi-worker SKIP LOCKED claim, lease expiry, retry and completion | each worker fills its batch from unlocked due rows; each lease has one owner; no job loss/duplication | concurrent MySQL integration | PARTIAL — IMPLEMENTATION/LOCAL REGRESSION PASS; MYSQL 8.4 REMOTE REVERIFY PENDING |
...
`34248233589` passed the Linux repository and LightRAG gates and verified the MySQL 8.4
escaped charset-introduced CHECK metadata fix. It then exposed an invalid transaction
isolation observation, valid optimizer plan variants and the old release-worker OR/filesort
lock footprint. The behavioral isolation check, paired index-prefix contracts and bounded
generated-`claimable_at` claim path now pass the complete local `make ci` gate but still
need the same remote integration gate. Production checks remain pending or
