...
| V39 | PRE_MERGE | rebuild fence | command-to-operation mapping, legal absent/stale/rebuilding/failed/verified transitions, abandoned attempt, atomic rename/fsync, canonical strict report schema, unknown/missing/inapplicable fields, digest tamper, generation/contract mismatch and concurrent runner | Python controller and Go readiness accept only the same complete verified report; `restore-verify` cannot be relabeled; exclusive lock and atomic state survive crashes | filesystem fault injection + Python/Go readiness tests | PASS — LOCAL |
