...
- Status: `IMPLEMENTED — LOCAL CI AND MYSQL 8.4 LIVE PASS; REDIS LIVE REVERIFY PENDING; PRE_MERGE BLOCKED`
...
| V17 | PRE_MERGE | release workers | generated claimable time axis, bounded multi-worker SKIP LOCKED claim, lease expiry, retry and completion | each worker fills its batch from unlocked due rows; each lease has one owner; no job loss/duplication | concurrent MySQL integration | PASS — MYSQL 8.4 RUN 34256811765 |
...
`34256811765` passed the Linux repository, LightRAG and complete MySQL 8.4 gates,
including the behavioral isolation check, paired index-prefix contracts and bounded
generated-`claimable_at` claim path. Redis live tests then exposed that an explicitly
closed future activity was still classified as `not_started`; the Lua fix uses the
persisted close marker as the terminal state while preserving replay precedence and now
passes focused local tests. It still needs the same remote integration gate. Production checks remain pending or
